"""Compact handling for ConnectFour."""
...
class CompactHandler:
 """Processes compact requests with retries and timeouts."""
 ...
 """Run a single compact operation."""
 started = time.time()
 attempts = 0
 last_error = None
 ...
 return {"compact": str(payload)[:200], "attempts": 0}
...
def run_compact(payload, **options):
 """Module-level convenience entry point."""
 return CompactHandler().run(payload, **options)